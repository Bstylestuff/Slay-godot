...
export(Texture) var dot_texture = preload('res://scenes/objects/ui/Chart/graph-plot-white.png')
export(Color) var default_chart_color = Color('#ccffffff')
export(Color) var grid_color = Color('#b111171c')
export(int, 'Pie') var chart_type = CHART_TYPE.PIE_CHART setget set_chart_type
...
func initialize(points_color = {}, animation_duration = 1.0):
...
func _draw_chart_background(pointListObject):
	for key in pointListObject.keys():
		...
		var color_alpha_ratio = COLOR_LINE_RATIO 
